 ...
 * Class Security
 *
 * Guardian domain A screen: security data quality.
 *
...
class Security extends \Opencart\System\Engine\Controller {
	...
	public function index(): void {
		...
		if (
			...
			|| !$this->model_extension_gtr_guardian_other_gtr_guardian->isDomainEnabled('security')
		) {
		...
		$this->load->language('extension/gtr_guardian/guardian/security');
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('extension/gtr_guardian/guardian/security', 'user_token=' . $this->session->data['user_token'])
		];
		...
		$this->response->setOutput($this->load->view('extension/gtr_guardian/guardian/security', $data));
	}
}
